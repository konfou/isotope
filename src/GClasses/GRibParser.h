...
public:
	...
	static GRayTraceScene* LoadScene(const char* szFilename);

	bool createScene(FILE* F);
	bool createScene(std::string filename);

	GRayTraceScene* DropScene()
	{
		GRayTraceScene* pScene = m_pScene;
		m_pScene = NULL;
		return pScene;
	}
...
private:
	...
	void setMaterial(GRayTraceMaterial* material);
	GRayTraceMaterial* getMaterial();
