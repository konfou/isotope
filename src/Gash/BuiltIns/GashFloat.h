...
#ifndef __GASHFLOAT_H__
#define __GASHFLOAT_H__

#include "../Include/GashEngine.h"
#include "GashString.h"
#include <math.h>

class GashFloat : public WrapperObject
{
friend class VarHolder;
public:
	double m_value;

	GashFloat(Engine* pEngine)
	...
	{
		pEngine->SetThis(new GashFloat(pEngine));
	}

	void newcopy(Engine* pEngine, EVar* pThat)
	{
		GashFloat* pNewThis = new GashFloat(pEngine);
