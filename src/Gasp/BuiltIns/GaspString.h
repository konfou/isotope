...
#ifndef __GASPSTRING_H__
#define __GASPSTRING_H__
...
public:
	...
	static GaspString* getConstString(Engine* pEngine, unsigned int nIndex)
	{
		const char* szAnsi = pEngine->GetLibrary()->GetStringFromTable(nIndex);
		GaspString* pThis = new GaspString(pEngine);
